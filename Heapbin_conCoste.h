#ifndef __HEAPBIN_CONCOSTE_H
#define __HEAPBIN_CONCOSTE_H
...
public:
	...
	*/
	long join(Heapbin h2) {
		_min = NULL;
		long coste = 0;
		/* mergesort entre los dos monticulos, los arboles estan ordenados por su grado de orden creciente*/
		_ra = merge(_ra, h2._ra, coste);

		if (_ra != NULL) {

			/** Existen 3 casos despues de realizar el mergesort:
				1) un unico arbol binario para un grado
				2) dos arboles binarios para un grado
				3) tres arboles binarios para grado k, uno de cada monticulo y el otro de haber enlazado dos arboles de grado k-1
				por lo tanto necesitamos comprobar las anteriores posibilidades*/
	...
	*/
	long decreaseKey(Nodo* nodo, const T& key) {
		int aux;
		nodo->_elem = key;
...
private:
	...
	*/
	static Nodo *merge(Nodo *ra1, Nodo *ra2, long &coste) {
		Nodo *ra = NULL;
